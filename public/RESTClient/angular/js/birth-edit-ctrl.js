      angular
          .module("BirthApp")
          .controller("BirthEditCtrl", ["$scope", "$http", "$routeParams", "$location", function($scope, $http, $routeParams, $location) {
              ...
              $scope.put = function() {
                  $http.put(baseURL + "/" + $routeParams.region + "/" + $routeParams.year, $scope.updatedBirth)
                      .then(function(response) {
                          $location.path("/");
                      });
              }
